...
class JacobiRobot:
    ...
    def servo_to_pose(
        ...
        计算带有速度和加速度限制的伺服驱动到目标位姿的关节速度。

        参数:
            target_pose: 表示期望位姿的 4x4 变换矩阵
            dt: 数值积分的时间步长
            linear_tol: 线性位置容差
            angular_tol: 角度方向容差

        返回:
            bool: 如果达到目标位姿返回 True，否则返回 False
        """
        # Get current pose
...
if __name__ == "__main__":
    ...
    urdf_path = "/home/kuanli/Documents/ros2_ws/src/mocap_ws/src/mocap/mocap_system/ik_solver/lite6.urdf"
    try:
        # Initialize robot
        robot = JacobiRobot(
