...
from mocap_system.visualize.base_visualizer import BaseVisualizer
...
class ViserVisualizer(BaseVisualizer):
    ...
    def display(self, q=None):
        ...
        if self.collision_model is not None and self.collision_data is not None and self.collisionRootFrame.visible:
            self.updatePlacements(pin.GeometryType.COLLISION)

        if self.visual_model is not None and self.visual_data is not None and self.visualRootFrame.visible:
            self.updatePlacements(pin.GeometryType.VISUAL)
    ...
    def updatePlacements(self, geometry_type):
        ...
        if geom_model is None or geom_data is None:
            return  # 防止NoneType传入
